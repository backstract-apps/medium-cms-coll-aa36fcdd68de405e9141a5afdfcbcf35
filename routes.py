...
@router.get('/persons/')
async def get_persons( RollNo: int , db: Session = Depends(get_db)):
    return await service.get_persons(db , RollNo)
...
async def delete_addresses_id( id: int, Name: str , db: Session = Depends(get_db)):
    return await service.delete_addresses_id(db , id, Name)
